function [h, class] = LMplot3D(varargin)
...
if isfield(annotation, 'object')
   ...
   for i = 1:Nobjects
       if isfield(annotation.object(i), 'polygon3D')
           if ~isempty(annotation.object(i).polygon3D)
               n = n+1;
               class{n} = annotation.object(i).name; % get object name
               col = colors(mod(sum(double(class{n})),15)+1, :);
               [X,Y,Z] = getLMpolygon3D(annotation.object(i).polygon3D);
               ...
               %plot3([X; X(1)],[Z; Z(1)], [Y; Y(1)], 'LineWidth', LineWidth, 'color', [0 0 0]); hold on
               h(n) = plot3([X; X(1)], [Z; Z(1)], [Y; Y(1)], 'LineWidth', LineWidth/2, 'color', col);
               hold on
           end
       end
   end
